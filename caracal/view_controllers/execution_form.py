...
from caracal.view_controllers.log_view import log_view
...
class execution_form(npyscreen.FormBaseNew):
    ...
    def start_pipeline(self):
        ...
        self.__pl_proc = caracal.execute_pipeline(
            cp().args, cp().arg_groups, block=False)
        Thread(target=__block_and_callback, args=(
            self.__pl_proc, self.on_pipeline_complete)).start()
