...
from caracal.view_controllers.message_boxes import input_box, message_box, warning_box, error_box, optioned_input_box
    ...
    def h_select(self, msg):
        ...
        def __uinput_factory(chain, db):
            ...
            def __numeric_input(val_type, chain, db):
                ...
                def __validate(x):
                    try:
                        ...
                        instance = error_box(
                            self.parent.event_loop, "Cannot convert {} to {}".format(x, val_type))
                        self.parent.event_loop.registerForm(
                            "MESSAGEBOX", instance)
            ...
            def __string_input(val_type, chain, db, valid_options, is_required):
                ...
                def __validate(x):
                    db.update_key(chain, x if not isinstance(
                        valid_options, list) else list(set(valid_options))[x[0]])
                    self.parent.rebuild_opt_tree()
                    self.display()
                        ...
                        for xi in list_vals:
                            if isinstance(valid_options, list):
                                if xi not in list(set(valid_options)):
                                    raise ValueError("'{}' is not an acceptable value. Requires one of {}.".format(
                                        xi, "[{}]".format(",".join(list(set(valid_options))))))
                            __cast = float if val_type == "float" else int if val_type == "int" else str
                            try:
                                results.append(__cast(xi))
                            except ValueError:
                                raise ValueError(
                                    "Cannot convert '{}' to {}".format(xi, val_type))
                        db.update_key(chain, results)
                        self.parent.rebuild_opt_tree()
                        self.display()
                    except ValueError as e:
                        instance = error_box(self.parent.event_loop, str(
                            e), minimum_columns=90, columns=90)
                        self.parent.event_loop.registerForm(
                            "MESSAGEBOX", instance)
                        self.parent.event_loop.switchForm("MESSAGEBOX")
...
class option_editor(npyscreen.FormBaseNew):
    ...
    def on_load(self, labeltype=npyscreen.TitleFilenameCombo, labeltext="Filename", editvalue=cp().args.config):
        ...
        instance = input_box(self.event_loop, labeltype, labeltext,
                             editvalue, columns=100, on_ok=on_confirm_load)
    ...
    def rebuild_opt_tree(self):
        def __populate_tree(groups, tree=None, level=1, level_label=1):
            ...
            def __isdict(x): return isinstance(
                x, dict) or isinstance(x, OrderedDict)

            if tree is None:
                if not __isdict(groups):
                    raise TypeError(
                        "Expected groups argument to be of type dictionary")
                tree = annotated_tree_data(
                    key_val=None, content='Root', selectable=False, ignoreRoot=True)

            ki = 1
            for k, v in sorted(iter(groups.items()), key=lambda k_v: k_v[1].get("order", 0) if hasattr(k_v[1], "get") else 0):
                if __isdict(groups[k]) and groups[k].get('enable', True):
                    label = "(X) {}. {}".format(
                        ki, k) if level == 1 else "(X) {}.{} {}".format(level_label, ki, k)
                    subtree = tree.newChild("@{}".format(
                        k), content=label, selectable=True)
                    ...
                    label = "( ) {}. {}".format(
                        ki, k) if level == 1 else "( ) {}.{} {}".format(level_label, ki, k)
                    subtree = tree.newChild("@{}".format(
                        k), content=label, selectable=True)
                    ki += 1
                else:
                    if k not in ["order", "enable"]:
                        label = "{}. {}".format(
                            ki, k) if level == 1 else "{}.{} {}".format(level_label, ki, k)
                        subtree = tree.newChild(
                            k, content="{} = {}".format(label, v), selectable=True)
                        ki += 1
            return tree
    ...
    def create(self):
        ...
        self.box_help = self.add(npyscreen.BoxBasic, name="Help",
                                 max_width=85, rely=4, relx=-89, max_height=20, editable=False)
        ...
        self.lbl_help = self.add(npyscreen.MultiLineEdit, value=msg,
                                 max_width=80, rely=6, relx=-83, max_height=16, editable=False)
        self.lbl_help.color = "SAFE"
        self.trv_options = self.add(
            opt_treeview, rely=4, width=80, exit_right=True, scroll_exit=True)
        self.rebuild_opt_tree()
