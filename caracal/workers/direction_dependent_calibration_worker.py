...
def worker(pipeline, recipe, config):
    ...
    def dd_precal_image(field,ms_list):
        ...
        output=OUTPUT,
        shared_memory="500gb",
        label="ddf_image_{0:s}:: DDFacet initial image for DD calibration".format(field))
        recipe.run()
        recipe.jobs = []
    def dd_postcal_image(field,ms_list):
        ...
        if (use_mask):
            ...
            output_folder = "/"+outdir
            recipe.add("cab/cleanmask", "mask_ddf_postcal_{0:s}".format(field),{
        ...
        for ms in mslist:
           ...
           recipe.add('cab/cubical', step, {
            ...
            #  "model-beam-m-axis" : "py",
              ...
              "data-time-chunk"   : ddsols_t*int(min(1,config[key].get('dist_nworker'))) if (ddols_f==0 or config[key].get('dd_g_channel_int')== 0) else ddsols_t*int(min(1,np.sqrt(config[key].get('dist_nworker')))),
              "data-freq-chunk"   : 0 if (ddols_f==0 or config[key].get('dd_g_channel_int')== 0) else ddsols_f*int(min(1, np.sqrt(config[key].get('dist_nworker')))),
