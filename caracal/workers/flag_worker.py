...
def worker(pipeline, recipe, config):
    ...
    mslist = pipeline.get_mslist(label, target=(config['field'] == "target"))
    target_ls = pipeline.target[obsid] if config['field'] == "target" else []
        ...
        if pipeline.enable_task(config, 'flag_time'):
            ...
            if config['flag_time']['ensure_valid']:
                if pipeline.startdate:
                    ...
                    if (flag_start <= pipeline.enddate[obsid]) and (pipeline.startdate <= flag_end):
                        found_valid_data = 1
                else:
            ...
            if ensure:
                if pipeline.startdate:
                    ...
                    for nn, time_range in enumerate(times):
                        ...
                        flag_start = float(''.join(re.split('/|:', start_flagrange)))
                        flag_end = float(''.join(re.split('/|:', end_flagrange)))
                        if (flag_start <= pipeline.enddate) and (pipeline.startdate <= flag_end):
                            found_valid_data[nn] = 1
                else:
                    raise ValueError("You wanted to ensure a valid time range but we could not find a start and end time")
            for nn, antenna in enumerate(antennas):
                antstep = 'ant-{0:s}-ms{1:d}-antsel{2:d}'.format(wname, obsid, nn)
            ...
            if pipeline.enable_task(config, 'inspect'):
                ...
                if config['field'] == 'target':
                    ...
                    field = ",".join(map(str, utils.get_field_id(msdict, manfields.get_field(
                        pipeline, obsid, config['inspect']['field']).split(","))))
                for f in field.split(','):
                    outlabel = '_{0:d}'.format(obsid) if len(field.split(',')) == 1 else '_{0:d}_{1:s}'.format(obsid, f)
