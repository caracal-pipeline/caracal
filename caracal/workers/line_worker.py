    ...
    for tt, target in enumerate(all_targets):
        ...
        if pipeline.enable_task(config, 'imcontsub'):
            ...
            # this is the option if the input datacube is provided by the user in the config file
                ...
                if any(p.exists() for p in cubepaths_to_check):
                    ...
                    caracal.log.error('Input datacube not found in output/ or output/cubes or output/cubes/cube_{0:d} please check your configuration file.'.format(maxcube_dir))
