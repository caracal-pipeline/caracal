...
def worker(pipeline, recipe, config):
    ...
    for tt, target in enumerate(all_targets):
        ...
        if pipeline.enable_task(config, 'imcontsub'):
            ...
            # caracal will look for the corresponding mask saved by sofia, if this does not exist imcontsub will use the automasking method
            if not config['imcontsub']['input_cube']:
                ...
                for uu in range(len(imsub_image_cube_list)):
                    ...
                    if config['imcontsub']['mask_image'] == 'sofia':                            
                        ...
                        if os.path.exists('{0:s}/cube_{1:d}/{2:s}'.format(pipeline.cubes,maxcube_dir,mask_name)):
                            ...
                            imcontsub_opts.update({"mask-image": '{0:s}/cube_{1:d}/{2:s}'.format(pipeline.cubes,maxcube_dir,mask_name) + ':input'})
                        else:
                    ...
                    imcontsub_opts.update({"infits": '{0:s}/cube_{1:d}/{2:s}'.format(pipeline.cubes,maxcube_dir,input_cube) + ':input'})
                ...
                caracal.log.info(
                    'Subtracted continuum in the image domain for target {0:d}'.format(tt))
            ...
            # this is the option if the input datacube is provided by the user in the config file
                ...
                if cubepath:
                    ...
                    caracal.log.error('Input datacube {0:1} not found in output/ or output/cubes or output/cubes/cube_{1:d} please check your configuration file.'.format(config['imcontsub']['input_cube'],maxcube_dir))
                ...
                caracal.log.info(
                    'Subtracted continuum in the image domain for datacube {0:s} provided by user '.format(config['imcontsub']['input_cube']))
