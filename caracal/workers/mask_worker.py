...
def worker(pipeline, recipe, config):
    ...
    def nvss_pbcorr(ra_deg, dec_deg, centre, cell, imsize, obs_freq, flux):
        ...
        pb_fwhm = 1.02 * (2.99792458e8) / obs_freq / 13.5 / np.pi * 180.0
    ...
    def query_catalog_sumss(catalog_table, centre, width_im, cat_name):
        ...
        p = Vizier.query_region(
            coord.SkyCoord(centre[0], centre[1], unit=(u.hourangle, u.deg), frame="icrs"),
            width=width_im,
            catalog=cat_name,
        )
        tab = p[0]
    ...
    def set_mosaic_files(catalog_table, mask_dir, fields_dir):
        ...
        if not os.path.exists(mosaic_tmpdir):
    ...
    def make_mask_nvss(catalog_table, centre, imsize, cell, mask):
        ...
        for i in range(0, len(pix_x)):
            ...
            if minor[i] / 3600.0 >= float(hdr["CDELT2"]) and major[i] / 3600.0 >= float(hdr["CDELT2"]):
                a = major[i] / 3600.0 / float(hdr["CDELT2"]) / 2.0
                b = minor[i] / 3600.0 / float(hdr["CDELT2"]) / 2.0
    ...
    for target in all_targets:
        if centre[0] == "HH:MM:SS" and centre[1] == "DD:MM:SS":
            ...
            coords = [targetpos[0] / np.pi * 180.0, targetpos[1] / np.pi * 180.0]
        ...
        if catalog_name == "SUMSS":
            ...
            if pipeline.enable_task(config, "make_mask"):
                if not pipeline.enable_task(config, "merge_with_extended"):
                    cat_mask = final_mask
                else:
        ...
        if pipeline.enable_task(config, "merge_with_extended"):
            ...
            if not os.path.exists(pipeline.output + "/" + beam):
                recipe.add(
                    build_beam,
                    "build_pb",
                    {
            ...
            if not os.path.exists(pipeline.output + "/" + beam_casa):
                recipe.add(
                    "cab/casa_importfits",
                    "import-pb",
                    {
            ...
            recipe.add(
                merge_masks,
                "make_mask-merge",  # 'Merging VLA Fornax into catalog mask',
                {
