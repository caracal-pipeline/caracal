...
def worker(pipeline, recipe, config):
    ...
    def identify_last_subdirectory(mosaictype):
        ...
        matching_subdirectories = glob.glob(
            directory_to_check + "/" + subdirectory_prefix + "*"
    ...
    def build_beam(obs_freq, centre, cell, imsize, out_beam):
        ...
        centre = coord.SkyCoord(centre[0], centre[1], unit=(u.deg, u.deg), frame="icrs")
        # cell /= 3600.0  # Am assuming that cell was passed to the function in units of
        #  arcsec, so this is converting it to units of deg.
        ...
        x, y = np.meshgrid(
            np.linspace(-hdr["NAXIS2"] / 2.0, hdr["NAXIS2"] / 2.0, hdr["NAXIS2"]),
        ...
        if len(cdelt3s) > 1:
            if nrdecimals:
                ...
                if len(cdelt3s_r) > 1:
                    ...
                    caracal.log.warn(
                        "Changing CDELT3 of all input image.fits and pb.fits cubes to {0:}".format(cdelt3s_r[0])
                    )
                    for ff in image_filenames:
                ...
                caracal.log.error(
                    "To proceed Please set mosaic:round_cdelt3 to round the CDELT3 values to an adequate "
                    "number of decimals."
                )
    ...
    if specified_mosaictype == "spectral":
        ...
        pb_origin = (
            "that are already in place (generated by the selfcal_worker, or during a previous run of the mosaic_worker)"
        )
    ...
    if not len(specified_images):
        ...
        caracal.log.info(
            "It is assumed that they are all in the highest-numbered subdirectory of "
            "'general:output/continuum' and 'general:output/cubes'."
        )
        caracal.log.info(
            "You should check the selected image names. If unhappy with the selection, "
    ...
    if found_stokes:
        ...
        caracal.log.error(
            "You may also want to disable any steps of the line worker that you do not need to repeat, e.g., make_cube."
        )
    ...
    for image_name in specified_images:
        ...
        if os.path.exists(pb_name):
            caracal.log.info("{0:s} is already in place, and will be used by montage_mosaic.".format(pb_name))
            ...
            if specified_mosaictype == "spectral":
                ...
                caracal.log.info("{0:s} does not exist, so going to create a pb.fits file instead.".format(pb_name))
                ...
                if pb_type == "gaussian":
                    ...
                    caracal.log.info(
                        "Observing frequency = {0:f} Hz, dish diameter = {1:f} m".format(
                    ...
                    if freq == 1383685546.875:  # i.e. if the default value was used
                        caracal.log.info(
                            ...
                            "with ref_frequency set in the config file."
                        )
                    else:
                        caracal.log.info(
                            "as set via ref_frequency in the config file, and used for primary-beam creation."
                        )
    ...
    for specified_image in specified_images:
        ...
        if not specified_image.split(input_directory)[0]:
            ...
            specified_image = "{0:s}/{1:s}".format(
                "/".join([".." for ss in input_directory.split("/")]), specified_image
            )
    ...
    if pipeline.enable_task(config, "domontage"):
        recipe.add(
            ...
            input=input_directory,
            output=pipeline.mosaics,
            label="MosaicSteward:: Re-gridding {0:s} images before mosaicking them. For this mode, "
            "the mosaic_worker is using *pb.fits files {1:s}.".format(specified_mosaictype, pb_origin),
        )
    ...
    caracal.log.info(
        "Setting BUNIT = {0:}, BMAJ = {1:}, BMIN = {2:}, BPA = {3:} in mosaic FITS headers".format(
            mosbunit, mosbmaj, mosbmin, mosbpa
        )
    )
