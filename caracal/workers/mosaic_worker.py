...
def worker(pipeline, recipe, config):
    ...
    def identify_last_subdirectory(mosaictype, prfx):
    ...
    if not len(specified_images):
        ...
        caracal.log.info("It is assumed that they are all in the highest-numbered subdirectory of 'general:output/continuum' and 'general:output/cubes'.")
    ...
    for image_name in specified_images:
        ...
        if os.path.exists(pb_name):
            ...
            if specified_mosaictype == "spectral":
                caracal.log.error("{0:s} does not exist. Please make sure that it is in place before proceeding.".format(pb_name))
