...
def worker(pipeline, recipe, config):
    ...
    msname = pipeline.msname
    msroot = pipeline.msbasename
    ...
    if pipeline.enable_task(config, 'obsinfo'):
        ...
        if config['obsinfo']['vampirisms']:
            step = 'vampirisms-ms{0:d}'.format(obsid)
        ...
        if pipeline.enable_task(config['obsinfo'], 'plotelev'):
            if os.path.exists(os.path.join(pipeline.msdir, elevplot)):
                ...
                step = "elevation-plots-ms{:d}".format(obsid)
    ...
    for line in content:
        ...
        if len(info_on_line) > 2:
            if info_on_line[0].lower() == 'observed' and info_on_line[1].lower() == 'from':
                ...
                pipeline.startdate = float(correct_date)
    ...
    pipeline.firstchanfreq = firstchanfreq
    pipeline.lastchanfreq = lastchanfreq
    pipeline.chanwidth = chanwidth
    ...
    # Get spectral frame
    pipeline.specframe = msdict['SPW']['MEAS_FREQ_REF']
    ...
    for term in "target gcal fcal bpcal xcal".split():
        conf_fields = getattr(pipeline, term)
        ...
        setattr(pipeline, term, list(found_fields))
        ...
        for f in found_fields:
            ...
            caracal.log.info(f'---')
        setattr(pipeline, term + "_ra", _ra)
        setattr(pipeline, term + "_dec", _dec)
        setattr(pipeline, term + "_id", _fid)
