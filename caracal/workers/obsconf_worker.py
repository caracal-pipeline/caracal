    ...
    for i, (msname, msroot) in enumerate(zip(pipeline.msnames, pipeline.msbasenames)):
        ...
        for term in "target gcal fcal bpcal xcal".split():
            ...
            if not found_fields:
                ...
                if term != "xcal":
                    raise RuntimeError(
                        f"Can't find an appropriate FIELD for obsinfo: {term}: {conf_fields_str}. "
                        "Please check this config setting. It may also be that your MS scan intents "
                        f"are not pupulated correctly, in which case you must set {term} to a list of"
