...
def xcal_model_xcal_leak(
    ...
    if not docal:
        ...
        meanchanfreq = (firstchanfreq[0] + lastchanfreq[0]) / 2.0 / 1.0e9
...
def xcal_from_pa_xcal_leak(
    ...
    applyfields = [
        field,
        ",".join(set(pipeline.fcal[i] + pipeline.bpcal[i] + pipeline.gcal[i] + pipeline.target[i])),
        "",
        "",
        "",
    ]
...
""",
        ...
        caracal.log.info("Polarization calibration selection: leakage_calib={leakage_calib}, pol_calib={pol_calib}")
