...
def worker(pipeline, recipe, config):
    ...
    mslist = pipeline.get_mslist(label, target=(field_name == "target"))
    ...
    for msname in mslist:
            ...
            for f in pipeline.bpcal:
                ...
                if fielddb is not None:
                    ...
                    if pipeline.enable_task(config, 'fixcalcoords'):
                        ...
                        step = 'fixuvw-ms{0:d}-{1:s}'.format(obsid, f)
        ...
        if pipeline.enable_task(config, "manage_flags"):
            ...
            if mode == "legacy":
                ...
                if version not in available_flagversions:
                    ...
                    step = "save-legacy-{0:s}-ms{1:d}".format(wname, obsid)
                    ...
                    version = "caracal_legacy"
                    step = "restore-flags-{0:s}-ms{1:d}".format(wname, obsid)
                    manflags.restore_cflags(pipeline, recipe, version,
                                            msname, cab_name=step)
                    if available_flagversions[-1] != version:
                        step = 'delete-flag_versions-after-{0:s}-ms{1:d}'.format(version, obsid)
        ...
        if pipeline.enable_task(config, "clearcal"):
            step = 'clearcal-ms{:d}'.format(obsid)
            fields = set(pipeline.fcal + pipeline.bpcal)
        ...
        if pipeline.enable_task(config, "specweights"):
            ...
            if specwts == "uniform":
                step = 'init_ws-ms{:d}'.format(obsid)
                ...
                step = 'calculate_ws-ms{:d}'.format(obsid)
                ...
                step = 'delete_ws-ms{:d}'.format(obsid)
                recipe.add('cab/casa_script', step,
                            {
                                "vis": msname,
