        ...
        for msname in mslist:
            ...
            if pipeline.enable_task(config, 'fixcalcoords'):
               ...
               for f in pipeline.bpcal[i]:
                   ...
                   if ra_corr:
                      caracal.log.info("The calibrator coordinates in the measurement set are incorrect.")
                      caracal.log.info("Brace for reprediction via fixvis.")
                      ra_corr = float(ra_corr*180.0/np.pi)
                      dec_corr = float(dec_corr*180.0/np.pi)
                      print("ra,dec", ra_corr, dec_corr)
                     ...
                     caracal.log.info("The coordinates of calibrator {0:s} are correct, no fixvis required.".format(f))
