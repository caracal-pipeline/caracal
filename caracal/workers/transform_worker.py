...
def worker(pipeline, recipe, config):
    ...
    if transform_mode == 'split':
        from_mslist = pipeline.get_mslist(label_in, target=from_target)
    elif transform_mode == 'concat':
        from_mslist = pipeline.get_mslist('', target=from_target)
    to_mslist = pipeline.get_mslist(label_out, target=not splitting_cals)

    # if splitting cals, we'll split one (combined) target to one output MS
    if splitting_cals:
        calfields = set()
        for fd in field_to_split:
            for elem in getattr(pipeline, fd):
                calfields.add(elem)
    ...
    for target_iter, (target, from_ms, to_ms) in enumerate(zip(target_ls, from_mslist, to_mslist)):
        ...
        if pipeline.enable_task(config, 'split_field'):
            step = 'split_field-ms{0:d}-{1:d}'.format(obsid, target_iter)
        ...
        if pipeline.enable_task(config, 'changecentre'):
            if config['changecentre']['ra'] == '' or config['changecentre']['dec'] == '':
                ...
                sys.exit(1)
            step = 'changecentre-ms{0:d}-{1:d}'.format(obsid, target_iter)
        ...
        if pipeline.enable_task(config, 'concat'):
            ...
            step = 'concat-ms{0:d}-{1:d}'.format(obsid, target_iter)
            ...
            step = 'singlespw-ms{0:d}-{1:d}'.format(obsid, target_iter)
            recipe.add('cab/casa_mstransform', step,
                       {
        ...
        if pipeline.enable_task(config, 'obsinfo'):
            if config['obsinfo']['listobs']:
                ...
                if pipeline.enable_task(config, 'split_field') or transform_mode == 'concat':
                    ...
                    listfile = '{0:s}-obsinfo.txt'.format(pipeline.msbasename)

                step = 'listobs-ms{0:d}-{1:d}'.format(obsid, target_iter)
            ...
            if config['obsinfo']['summary_json']:
                ...
                if pipeline.enable_task(config, 'split_field') or transform_mode == 'concat':
                    ...
                    listfile = '{0:s}-summary.json'.format(pipeline.msbasename)

                step = 'summary_json-ms{0:d}-{1:d}'.format(obsid, target_iter)
