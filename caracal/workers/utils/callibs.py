...
def resolve_calibration_library(
    pipeline, msprefix, cal_lib, cal_label, output_fields=None, default_interpolation_types={}
):
    """
    ...
    Arguments:
        ...
        default_interpolation_types:
                        interpolation types for default fields, as a mapping
                        e.g. {'delay_cal': 'nearest', 'gain_cal': 'linear'}, which
    ...
    with open(outfile, "w") as stdw:
        for gain_type, cal_entries in caldict.items():
            ...
            for field, entry in cal_entries.items():
                ...
                stdw.write(
                    f"""caltable="{filename}" calwt={calwt} tinterp='{interp}' """
