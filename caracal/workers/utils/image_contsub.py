...
def imcontsub(
    ...
    if not isinstance(mask, type(None)):
        ...
        incubus_data_masked = np.ma.masked_array(incubus_data, (mask_data > 0) + np.isnan(incubus_data))
        hdul_mask.close()
    else:
        incubus_data_masked = np.ma.masked_array(incubus_data, np.isnan(incubus_data))
    ...
    if fitmode == "poly":
        ...
        if np.nanmax(incubus_data) > 0.0:
            ...
            maxincube = 0
        if np.nanmin(incubus_data) < 0.0:
            minincube = np.nanmin(incubus_data) * 10.0
        else:
            minincube = 0.0
        ...
        fit[np.logical_not(np.isfinite(fit))] = 0.0
        ...
        if length == 0:
            ...
            if sgiters > 0:
                fit = scipy.ndimage.median_filter(sgincubus, (length, 1, 1))
                ...
                for i in range(sgiters):
                    ...
                    fit = scipy_signal.savgol_filter(sgincubus, length, polyorder, axis=0, mode="interp")
            else:
                fit = scipy_signal.savgol_filter(sgincubus, length, polyorder, axis=0, mode="interp")
    ...
    if kersiz > 0:
        ...
        kernel = np.outer(kernel, kernel).reshape((1, kernel.size, kernel.size))
        kernel = np.repeat(kernel, fit.shape[0], axis=0)
        fitmask = np.isnan(fit)
        fit[fitmask] = 0.0
        convolved = scipy_signal.fftconvolve(fit, kernel, mode="same", axes=(1, 2)) / kernel[0].sum()
...
def parsing():
    ...
    # Common
    parser.add_argument("--incubus", "-i", help="Name of input data cube.", type=str)
    parser.add_argument("--outcubus", "-o", help="Name of continuum-subtracted output data cube", type=str)
    parser.add_argument("--fitmode", "-f", help="Type of fit ('poly' or 'savgol')", type=str)
    parser.add_argument("--length", "-l", help="Length of the sliding window in channels", type=str)
    parser.add_argument("--polyorder", "-p", help="Polynomial order", type=str)
    parser.add_argument(
        "--mask",
        "-m",
        help="Mask cube indicating regions to be excluded; excluded are voxels where the mask cube is not 0",
        type=str,
    )
    ...
    parser.add_argument("--clobber", "-c", help="overwrite output if set", default=False, action="store_true")
