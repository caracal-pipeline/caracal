...
def conflict(conflict_type, pipeline, wname, ms, config, flags_bw, flags_aw, read_version="version"):
    ...
    if conflict_type == "would_overwrite_bw" or conflict_type == "rewind_too_little":
        ...
        log.error("---------------------------------------------------------------------------------------------------")
        log.error(
            'A worker named "{0:s}" was already run on the .MS file {1:s} with pipeline prefix "{2:s}".'.format(
                wname, ms, pipeline.prefix
            )
        )
        if conflict_type == "rewind_too_little":
            log.error("and you are rewinding to a later flag version: {0:s} .".format(req_version))
        log.error(
            'Running "{0:s}" again will attempt to overwrite existing flag versions, it might get messy.'.format(wname)
        )
        log.error(
            'Caracal will not overwrite the "{0:s}" flag versions unless you explicitely request that.'.format(wname)
        )
        ...
        log.error(
            '    2) If you want to discard the flags obtained during the previous run of "{0:s}" (and,'.format(wname)
        )
        log.error(
            '       necessarily, all flags obtained thereafter; see list above) reset the "{0:s}" worker'.format(wname)
        )
        ...
        log.error(
            '    3) If you want to discard the flags obtained during the previous run of "{0:s}" and'.format(wname)
        )
        ...
        log.error(
            '       The worker "{0:s}" will be run again; the new flags will be appended to the current'.format(wname)
        )
        ...
        log.error("---------------------------------------------------------------------------------------------------")
        log.error(
            'Warning - Your choice will be applied to all .MS files being processed by the worker "{0:s}".'.format(
                wname
            )
        )
        ...
        log.error(
            'You have asked to rewind the flags of {0:s} to the version "{1:s}" but this version'.format(
                ms, req_version
            )
        )
        ...
        for vv in av_flagversions:
            log.error("       {0:s}".format(vv))
        log.error(
            "Note that if you are running Caracal on multiple targets and/or .MS files you should rewind to a flag"
        )
