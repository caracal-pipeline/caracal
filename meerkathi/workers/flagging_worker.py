    ...
    for i in range(nobs):
        ...
        if pipeline.enable_task(config, 'autoflag_rfi'):
            ...
            if config['autoflag_rfi'].get('fields', 'auto') != 'auto' and \
               not set(config['autoflag_rfi'].get('fields', 'auto').split(',')) <= set(['fcal','gcal', 'bpcal', 'target']):
                raise KeyError("autoflag rfi can only be 'auto' or be a combination of 'fcal', 'gcal', 'bpcal' or 'target'")
            def_fields = ','.join([pipeline.fcal_id[i], pipeline.bpcal_id[i], pipeline.gcal_id[i]]) if \
                         pipeline.bpcal_id[i] != pipeline.fcal_id[i] else \
                        ','.join([pipeline.bpcal_id[i], pipeline.gcal_id[i]])
