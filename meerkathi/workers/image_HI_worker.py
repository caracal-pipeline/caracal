    ...
    if not os.path.exists(filename):
        ...
        with fits.open(filename, mode='update') as cube:
            ...
            if 'FREQ' in headcube['ctype3'] and not reverse:
                ...
                # FITS standard for radio velocity as per https://fits.gsfc.nasa.gov/standard40/fits_standard40aa-le.pdf
...
def fix_specsys(filename, specframe):
    ...
    specsys3 = {0: 'LSRD', 1: 'LSRK', 2: 'GALACTOC', 3: 'BARYCENT',
                4: 'GEOCENTR', 5: 'TOPOCENT'}[np.unique(np.array(specframe))[0]]
    filename = filename.split(':')
    filename = '{0:s}/{1:s}'.format(filename[1], filename[0])
    if not os.path.exists(filename):
        meerkathi.log.info(
...
def make_pb_cube(filename):
    ...
    if not os.path.exists(filename):
        ...
        with fits.open(filename) as cube:
            headcube = cube[0].header
            datacube = np.indices(
                (headcube['naxis2'], headcube['naxis1']), dtype=float32)
            ...
            datacube = np.repeat(
                datacube, headcube['naxis3'], axis=0)*np.abs(headcube['cdelt1'])
...
def worker(pipeline, recipe, config):
    ...
    pipeline.prefixes = [
        '{2:s}-{0:s}-{1:s}'.format(did, config['label'], pipeline.prefix) for did in pipeline.dataid]
    ...
    for i, msfile in enumerate(all_msfiles):
        ...
        with open(msinfo, 'r') as stdr:
            ...
            Dec.append(tDec)
        meerkathi.log.info(
            'Target RA, Dec for Doppler correction: {0:.3f} deg, {1:.3f} deg'.format(RA[i], Dec[i]))
    ...
    if pipeline.enable_task(config, 'mstransform') and config['mstransform'].get('doppler') and config['mstransform'].get('outchangrid') == 'auto':
            ...
            with open(msinfo, 'r') as searchfile:
                for longdatexp in searchfile:
                    if "Observed from" in longdatexp:
                        dates = longdatexp
                        matches = re.findall(
                            '(\d{2}[- ](\d{2}|January|Jan|February|Feb|March|Mar|April|Apr|May|May|June|Jun|July|Jul|August|Aug|September|Sep|October|Oct|November|Nov|December|Dec)[\- ]\d{2,4})', dates)
                        ...
                        if corr_order:
                            if chanw_dopp[0] > 0.:
                                firstchanfreq_dopp[i], chanw_dopp[i], lastchanfreq_dopp[i] = lastchanfreq_dopp[i] * \
                                    corr, chanw_dopp[i] * \
                                    corr, firstchanfreq_dopp[i] * corr
                            else:
                                firstchanfreq_dopp[i], chanw_dopp[i], lastchanfreq_dopp[i] = firstchanfreq_dopp[i] * \
                                    corr, chanw_dopp[i] * \
                                    corr, lastchanfreq_dopp[i] * corr
                        else:
                            firstchanfreq_dopp[i], chanw_dopp[i], lastchanfreq_dopp[i] = firstchanfreq_dopp[i] * \
                                corr, chanw_dopp[i] * \
                                corr, lastchanfreq_dopp[i]*corr  # Hz, Hz, Hz
        ...
        if np.min(chanw_dopp) < 0:
            ...
            # safety measure to avoid wrong Doppler settings due to change of Doppler correction during a day
            comfreq0 += comchanw
            # safety measure to avoid wrong Doppler settings due to change of Doppler correction during a day
            comfreql -= comchanw
        else:
        ...
        meerkathi.log.info(
            'Calculated common Doppler-corrected channel grid for all input .MS: {0:d} channels starting at {1:s} and with channel width {2:s}.'.format(nchan_dopp, comfreq0, comchanw))
        ...
        if len(config['mstransform']['outchangrid'].split(',')) != 3:
            ...
            meerkathi.log.error('Current setting is mstransform:outchangrid:"{0:s}"'.format(
        ...
        meerkathi.log.info(
            'Set requested Doppler-corrected channel grid for all input .MS: {0:d} channels starting at {1:s} and with channel width {2:s}.'.format(nchan_dopp, comfreq0, comchanw))
    ...
    for i, msname in enumerate(all_msfiles):
        ...
        if pipeline.enable_task(config, 'mstransform'):
            if os.path.exists('{1:s}/{0:s}'.format(msname_mst, pipeline.msdir)):
                os.system(
    ...
    if pipeline.enable_task(config, 'wsclean_image'):
        ...
        if nchans == 0 or nchans == 'all':
            # Assuming user wants same spw for all msfiles and they have same number of channels
            nchans = nchans_all[0][spwid]
        # Assuming user wants same spw for all msfiles and they have same specframe
        ...
        if config['wsclean_image'].get('weight') == 'briggs':
            weight = 'briggs {0:.3f}'.format(
                config['wsclean_image'].get('robust', robust))
        else:
        ...
        for target in (all_targets):
            ...
            for j in range(1, wscl_niter+1):
                if j == 1:
                    ...
                    if ownHIclean_mask:
                        HI_image_opts.update(
                            {"fitsmask": ownHIclean_mask+':output/masking'})
                    ...
                    cubename = os.path.join(
                        pipeline.output, pipeline.prefix+'_'+field+'_HI_'+str(j)+'.image.fits')
                    ...
                    HIclean_mask = pipeline.prefix+'_'+field + \
                        '_HI_'+str(j)+'.image_clean_mask.fits:output'
                    HIclean_mask_path = os.path.join(
                        pipeline.output, pipeline.prefix+'_'+field+'_HI_'+str(j)+'.image_clean_mask.fits')

                    cubename = pipeline.prefix+'_'+field + \
                        '_HI_'+str(j-1)+'.image.fits:output'
                    cubename_path = os.path.join(
                        pipeline.output, pipeline.prefix+'_'+field+'_HI_'+str(j-1)+'.image.fits')
                    outmask = pipeline.prefix+'_' + \
                        field+'_HI_'+str(j)+'.image_clean'
                    ...
                    if not os.path.exists(HIclean_mask_path):
                        ...
                        for ss in ['dirty', 'psf', 'first-residual', 'residual', 'model', 'image']:
                            cubename = pipeline.prefix+'_'+field+'_HI_' + \
                                str(j)+'.'+ss+'.fits:'+pipeline.output
                            MFScubename = os.path.join(
                                pipeline.output, pipeline.prefix+'_'+field+'_HI_'+str(j)+'-MFS-'+ss+'.fits')
                    ...
                    # if the noise changes by less than 10% keep_going
                    if rms2 <= (1.0 - tol)*rms_ref:
                        rms_ref = rms2
                        if j == wscl_niter:
                            meerkathi.log.info('The relative noise change is larger than ' + str(
                                "{0:.3f}".format((100.0 - (rms2/rms_ref)*100.0))+'%. Noise convergence not achieved.'))
                            meerkathi.log.info(
                        ...
                        if rms2 >= (1.0 - tol)*rms_ref:
                            meerkathi.log.info('The relative noise change is less than '+str(
                                "{0:.3f}".format((100.0 - (rms2/rms_ref)*100.0))+'%. Noise convergence achieved.'))
                        break
            ...
            for ss in ['dirty', 'psf', 'first-residual', 'residual', 'model', 'image']:
                ...
                cubename = os.path.join(
                    pipeline.output, pipeline.prefix+'_'+field+'_HI_'+str(j)+'.'+ss+'.fits')
                finalcubename = os.path.join(
                    pipeline.output, pipeline.prefix+'_'+field+'_HI.'+ss+'.fits')
                HIclean_mask = os.path.join(
                    pipeline.output, pipeline.prefix+'_'+field+'_HI_'+str(j)+'.image_clean_mask.fits')
                finalHIclean_mask = os.path.join(
                    pipeline.output, pipeline.prefix+'_'+field+'_HI.image_clean_mask.fits')
                MFScubename = os.path.join(
                    pipeline.output, pipeline.prefix+'_'+field+'_HI_'+str(j)+'-MFS-'+ss+'.fits')
                MFSfinalcubename = os.path.join(
                    pipeline.output, pipeline.prefix+'_'+field+'_HI'+'-MFS-'+ss+'.fits')
