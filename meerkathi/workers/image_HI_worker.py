...
def worker(pipeline, recipe, config):
    ...
    for i, msfile in enumerate(all_msfiles):
        ...
        with open(msinfo, 'r') as stdr:
            ...
            targetpos=tinfo['REFERENCE_DIR']#[tinfo['NAME'].index(pipeline.target[i].split(',')[0])][0]
    ...
    if pipeline.enable_task(config, 'wsclean_image'):
        firstchanfreq_all,chanw_all,lastchanfreq_all,nchans_all,specframe_all = [],[],[],[],[]
        label = config['label']
        if label != '':
            flabel = '-'+label
        else: flabel = label
        ...
        for target in all_targets:
                ...
                for target in all_targets:
                    ...
                    # If channelisation changed during a previous pipeline run as stored in the obsinfo.json file
                        if not pipeline.enable_task(config, 'mstransform'):
                            ...
                            nchans[i] = [nchan_dopp for kk in nchans[i]]
                            specframe[i]=[{'lsrd':0,'lsrk':1,'galacto':2,'bary':3,'geo':4,'topo':5}[config['mstransform'].get('outframe', 'bary')] for kk in nchans[i]]
            ...
            if nchans == 0 or nchans == 'all': nchans=nchans_all[0][0]
    ...
    if pipeline.enable_task(config, 'casa_image'):
        ...
        for target in all_targets:
            ...
            if config['casa_image'].get('use_mstransform',True):
                label = config['label']
                if label != '':
                    flabel = '-'+label
                else: flabel = label
            ...
            image_opts = {
                 "msname"         :    mslist,
                 "prefix"         :    pipeline.prefix+'_'+field+'_HI_blaaaa',
#                 "field"          :    target,
                 "mode"           :    'channel',
                 "nchan"          :    nchans,
...
#                 "wprojplanes"    :    1,
            ...
            recipe.add('cab/casa_clean', step, image_opts,
               ...
               spwid = config['casa_image'].get('spwid', 0)
               nchans = config['casa_image'].get('nchans', 0)
               if nchans == 0 or nchans == 'all': nchans=nchans[0]
               step = 'casa_clean_image_HI'
               recipe.add('cab/casa_clean', step,
                   {
                        "msname"         :    mslist,
...
#                       "wprojplanes"    :    1,
     ...
     if config['casa_image'].get('taper', '') != '':
               ...
               #print pipeline.prefix+'_'+field+'_'+field, "blazzzz9"
...
#print targetpos,v,corr,chanw_dopp[0]

                #print firstchanfreq_dopp,chanw_dopp,lastchanfreq_dopps
                #sprint RA, Dec, firstchanfreq, lastchanfreq,  chanwidth
