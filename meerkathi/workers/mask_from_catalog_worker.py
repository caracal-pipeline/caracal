...
NAME = 'Make mask from SUMMS catalog include, if needed, Fornax A from VLA observations'
...
def worker(pipeline, recipe, config):
    ...
    def query_SUMMS(centre,width_im,catalog_table):
        
        Vizier.ROW_LIMIT = -1

        p = Vizier.query_region(coord.SkyCoord(centre[0], centre[1], unit=(u.hourangle, u.deg), frame='icrs'), width='2.8d', catalog='SUMSS')
        tab = p[0]
...
#	del head['CUNIT4']
    ...
    if pipeline.enable_task(config, 'query_SUMMS'):
        key = 'query_SUMMS'
        ...
        recipe.add(query_SUMMS, 'query_summs_catalog', 
            {   
              'centre'  : centre,
