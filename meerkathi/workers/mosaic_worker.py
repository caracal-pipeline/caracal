...
def worker(pipeline, recipe, config):
    ...
    if specified_mosaictype == 'spectral':
        label = 'corr' #pipeline.config['image_line'].get('label', 'corr') ### Did not work as expected. Come back this 
        ...
        pb_origin = 'generated by the self_cal_worker'
    ...
    if specified_images[0] == 'directory/first_image.fits': # If nothing is passed via the config file, then specified_images[0] adopts this via the schema
    ...
    for image_name in specified_images: 
        ...
        if os.path.exists(pathname + '/' + pb_name):
            ...
            if specified_mosaictype == 'spectral':
                meerkathi.log.error('{0:s}/{1:s} does not exist. Please make sure that it is in place before proceeding.'.format(pathname,pb_name))
                meerkathi.log.error('You may need to re-run the image_line worker with pb_cube enabled. EXITING.')
                sys.exit(1)
