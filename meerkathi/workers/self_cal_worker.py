...
#          label='{0:s}:: Cleanup SoFiA masks'.format(step))
    ...
    def calibrate_cubical(num, mslist, field):
        ...
        if config[key].get('output_data')[num-1 if len(config[key].get('output_data')) >= num else -1]  == 'CORR_DATA':
            flags= 'legacy'
        else:
            flags='-cubical'
        for i,msname in enumerate(mslist):
...
#                 "out-overwrite"   : False,
    ...
    def restore(num, mslist_out,enable_inter=True):
        ...
        for i,msname_out in enumerate(mslist_out):
            ...
            cubical_gain_interp_opts = {
               ...
               "sel-ddid"         : sdm.dismissable(config[key].get('spwid')),
               "dist-ncpu"        : ncpu,
               "out-name"         : '{0:s}-{1:s}_{2:d}_restore_cubical'.format(prefix, msname_out, num), # '{0:s}-{1:d}_cubical'.format(pipeline.dataid[i], apply_iter),
               "data-freq-chunk"  : 0,
               "out-mode"         : apmode,
               "weight-column"    : config[key].get('weight_column'),
               "montblanc-dtype"  : 'float',
    ...
    for target in all_targets:
        ...
        if pipeline.enable_task(config, 'transfer_apply_gains'):
            ...
            if (self_cal_iter_counter > cal_niter):
                restore(self_cal_iter_counter-1, mslist_out, enable_inter=True)
            else:
                restore(self_cal_iter_counter, mslist_out, enable_inter=True)
